...
exports.markLectureAsComplete = async (req, res) => {
    ...
    console.log("progressAlreadyExists : ", progressAlreadyExists)
	  ...
	  let completedVideos;
	  if (progressAlreadyExists && progressAlreadyExists.completeVideos) {
			completedVideos = progressAlreadyExists.completeVideos;}
	  ...
	  if (!completedVideos?.includes(subSectionId)) {
		await CourseProgress.findOneAndUpdate(
		  ...
		  {
			$push: { completeVideos: subSectionId },
		  }
		)
	  }else{
		return res.status(400).json({
	  ...
	  await CourseProgress.findOneAndUpdate(
		...
		{
		  completeVideos: completedVideos,
		}
	  )
	return res.status(200).json({
	  success: true,
